...
//                ->count();
        ...
        if(!empty($sales))
        {
            foreach ($sales as $sale)
            {
                ...
                if($sale->paidBalance!=0.00)
                {
                    $nestedData['action'] = '<a href="javascript:void(0);"  class=" btn btn-primary btn-sm"><i style="font-size: 20px" class="fa fa-ban"></i></a>&nbsp;<a href="'.url('sales_delete', $sale->id).'" onclick="return ConfirmDelete()"  class="btn btn-danger btn-sm"><i style="font-size: 20px" class="fa fa-trash"></i></i></a>';
                }
                else
                {
                    $nestedData['action'] = '<a href="'.route('sales.edit', $sale->id).'"  class=" btn btn-primary btn-sm"><i style="font-size: 20px" class="fa fa-edit"></i></a>&nbsp;<a href="'.url('sales_delete', $sale->id).'" onclick="return ConfirmDelete()"  class="btn btn-danger btn-sm"><i style="font-size: 20px" class="fa fa-trash"></i></i></a>';
                }
                $data[] = $nestedData;
            }
        }
        ...
        if($data)
        {
            ...
            if($data->paidBalance==0.00)
            {
                DB::transaction(function () use($data)
                {
                    ...
                    SaleDetail::where('sale_id', '=', $data->id)->where('company_id', '=', $company_id)->update(['user_id' => $user_id]);
                    SaleDetail::where('sale_id', '=', $data->id)->where('company_id', '=', $company_id)->delete();

                    AccountTransaction::where('Description', 'like', 'Sales|' . $data->id)->where('company_id', '=', $company_id)->update(['user_id' => $user_id,'updateDescription'=>'hide']);
                    AccountTransaction::where('Description', 'like', 'Sales|' . $data->id)->where('company_id', '=', $company_id)->delete();
